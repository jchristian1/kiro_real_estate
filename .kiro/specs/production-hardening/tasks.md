...
- [x] 10. Lead state machine consolidation
  ...
  - [x] 10.3 Create `GET /api/v1/agent/leads/{lead_id}/events` endpoint in `api/routers/agent_leads.py`
...
- [x] 12. Watcher/worker resilience
  ...
  - [x] 12.5 Implement auto-restart cooldown: when `ENABLE_AUTO_RESTART=true`, schedule watcher restart after 60-second cooldown following `FAILED` transition; log ERROR with `agent_id`, error type, message, and timestamp when marking `FAILED`
    - _Requirements: 10.2, 10.3_

  - [x] 12.6 Write property test for exponential backoff schedule
...
- [x] 14. Security hardening
  ...
  - [x] 14.2 Add `slowapi` rate limiting to `POST /api/v1/auth/login` and `POST /api/v1/agent/auth/login`
