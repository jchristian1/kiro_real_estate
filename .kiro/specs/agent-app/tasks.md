...
- [ ] 1. Database schema and Alembic migrations
  - [ ] 1.1 Create SQLAlchemy models for `agent_users`, `agent_sessions`, `agent_preferences`, `buyer_automation_configs`, `agent_templates`, `lead_events`
...
- [ ] 2. Credential encryption service
  - [ ] 2.1 Implement `encrypt_app_password` and `decrypt_app_password` using AES-256 (cryptography library), key from environment variable
...
- [ ] 3. Agent auth backend
  - [ ] 3.1 Implement `POST /api/v1/agent/auth/signup` — create agent account, auto-login, return session cookie
