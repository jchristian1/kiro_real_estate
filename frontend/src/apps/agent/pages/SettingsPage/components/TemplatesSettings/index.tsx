...
function renderPreview(text: string | undefined | null): string {
  if (!text) return '';
  return text
    .replace(/\{lead_name\}/g, SAMPLE_LEAD_DATA.lead_name)
    .replace(/\{agent_name\}/g, SAMPLE_LEAD_DATA.agent_name)
    .replace(/\{agent_phone\}/g, SAMPLE_LEAD_DATA.agent_phone)
    .replace(/\{agent_email\}/g, SAMPLE_LEAD_DATA.agent_email)
    .replace(/\{form_link\}/g, SAMPLE_LEAD_DATA.form_link);
}
        ...
        <div style={{ background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: 16, padding: '24px' }}>
          ...
          {showPreview && (
            <div style={{ marginBottom: 16, padding: '16px', background: t.bgPage, borderRadius: 10, border: `1px solid ${t.border}` }}>
              ...
              <div style={{ fontSize: 12, color: t.textFaint, marginBottom: 12 }}>Lead: {SAMPLE_LEAD_DATA.lead_name} · Agent: {SAMPLE_LEAD_DATA.agent_name}</div>
                ...
                <div style={{
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 3, flexWrap: 'wrap' }}>
                      <span style={{ fontSize: 10, fontWeight: 700, color: step.color, background: step.colorBg, padding: '2px 7px', borderRadius: 20 }}>STEP {step.step}</span>
                      <span style={{ fontSize: 14, fontWeight: 700, color: t.text }}>{step.title}</span>
                      {usingDefault ? (
                        <span style={{ fontSize: 10, fontWeight: 600, color: t.textFaint, background: t.bgBadge, padding: '2px 7px', borderRadius: 20 }}>Using default</span>
                      ) : (
                        <span style={{ fontSize: 10, fontWeight: 700, color: '#10b981', background: 'rgba(16,185,129,0.12)', padding: '2px 7px', borderRadius: 20 }}>✓ Active</span>
                      )}
                    </div>
                    <div style={{ fontSize: 12, color: t.textFaint, marginBottom: 4 }}><span style={{ fontWeight: 600 }}>When: </span>{step.trigger}</div>
                    <div style={{ fontSize: 12, color: t.textMuted, lineHeight: 1.5 }}>{step.description}</div>
                  </div>

                  <div style={{ display: 'flex', gap: 8, flexShrink: 0, alignItems: 'center' }}>
                    <button onClick={() => toggleExpand(step.type)} style={{ padding: '6px 14px', background: t.bgPage, border: `1px solid ${t.border}`, borderRadius: 8, fontSize: 12, color: t.textMuted, cursor: 'pointer' }}>{expandedId === step.type ? 'Hide' : 'Manage'}</button>
                    <button onClick={() => openNew(step.type)} style={{ padding: '6px 14px', background: 'linear-gradient(135deg, #6366f1, #8b5cf6)', border: 'none', borderRadius: 8, fontSize: 12, fontWeight: 600, color: '#fff', cursor: 'pointer' }}>+ New</button>
                  </div>
                </div>

                {expandedId === step.type && (
                  <div style={{ padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {platformDefault && <TemplateRow name="Platform Default" subject={platformDefault.subject} body={platformDefault.body} isActive={platformDefault.is_active} isReadOnly t={t} />}
                    {dbTemplates.map(tpl => <TemplateRow key={tpl.id} name={tpl.name || 'My Template'} subject={tpl.subject} body={tpl.body} isActive={tpl.is_active} version={tpl.version} onActivate={!tpl.is_active && tpl.id ? () => handleActivate(tpl.id!) : undefined} onEdit={() => openEdit(tpl)} onDelete={tpl.id ? () => handleDelete(tpl.id!) : undefined} t={t} />)}
                  </div>
                )}
              </div>
              {!isLast && <div style={{ display: 'flex', justifyContent: 'center', padding: '6px 0' }}><div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}><div style={{ width: 1, height: 12, background: t.border }} /><div style={{ fontSize: 12, color: t.textFaint }}>↓</div></div></div>}
            </div>
          );
        })}
      </div>
...
const TemplateRow: React.FC<any> = ({ name, subject, body, isActive, isReadOnly, version, onActivate, onEdit, onDelete, t }) => {
  const [expanded, setExpanded] = useState(false);
  return (
    <div style={{ background: t.bgPage, border: `1px solid ${isActive ? '#10b98140' : t.border}`, borderRadius: 10, overflow: 'hidden' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px' }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 2 }}>
            {isActive && <span style={{ fontSize: 10, fontWeight: 700, color: '#10b981', background: 'rgba(16,185,129,0.12)', padding: '1px 7px', borderRadius: 20 }}>✓ Active</span>}
            <span style={{ fontSize: 13, fontWeight: 600, color: t.text }}>{name}</span>
          </div>
          <div style={{ fontSize: 12, color: t.textMuted, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{subject}</div>
        </div>
        <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
          <button onClick={() => setExpanded(v => !v)} style={{ padding: '4px 10px', background: 'none', border: `1px solid ${t.border}`, borderRadius: 7, fontSize: 11, color: t.textMuted, cursor: 'pointer' }}>{expanded ? 'Hide' : 'Preview'}</button>
          {onActivate && <button onClick={onActivate} style={{ padding: '4px 10px', background: t.accentBg, border: `1px solid ${t.accent}40`, borderRadius: 7, fontSize: 11, fontWeight: 600, color: t.accent, cursor: 'pointer' }}>Use This</button>}
          {!isReadOnly && onEdit && <button onClick={onEdit} style={{ padding: '4px 10px', background: 'none', border: `1px solid ${t.border}`, borderRadius: 7, fontSize: 11, color: t.textMuted, cursor: 'pointer' }}>Edit</button>}
          {!isReadOnly && onDelete && <button onClick={onDelete} style={{ padding: '4px 10px', background: t.redBg, border: `1px solid ${t.red}30`, borderRadius: 7, fontSize: 11, color: t.red, cursor: 'pointer' }}>Delete</button>}
        </div>
      </div>
      {expanded && (
