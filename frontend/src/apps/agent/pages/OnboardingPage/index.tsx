/**
 * OnboardingPage — 7-step wizard shell (steps 0–6).
...
import { ONBOARDING_STEPS } from '@/models/agent';
import {
  Step0Account,
  Step1Profile,
  Step2Gmail,
...
export const OnboardingPage: React.FC = () => {
  ...
  const currentStep = ONBOARDING_STEPS.findIndex(s => s.path === seg);
  ...
  const goBack = () => {
    if (stepIndex > 0) navigate(`/agent/onboarding/${ONBOARDING_STEPS[stepIndex - 1].path}`);
  };
        ...
        <div style={{ marginBottom: 8 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
            <span style={{ fontSize: 12, color: t.textMuted }}>
              Step {stepIndex + 1} of {ONBOARDING_STEPS.length} — {ONBOARDING_STEPS[stepIndex]?.label}
            </span>
