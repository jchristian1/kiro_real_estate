...
export const AgentReportsPage: React.FC = () => {
  ...
  const cardStyle = {
    background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: 14, padding: '16px',
  };
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 16, flexWrap: 'wrap' }}>
        ...
        {data && (
          <span style={{ fontSize: 11, color: t.textFaint, alignSelf: 'center', marginLeft: 6 }}>
      ...
      {isLoading ? (
        <div style={{ textAlign: 'center', padding: '32px 0', color: t.textMuted, fontSize: 13 }}>Loading reports…</div>
      ) : !data ? null : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 8 }}>
          {/* Summary stats */}
          <div style={{ ...cardStyle, gridColumn: '1 / -1', display: 'flex', gap: 16, flexWrap: 'wrap' }}>
            {[
              ...
              <div key={stat.label} style={{ flex: 1, minWidth: 100 }}>
                <div style={{ fontSize: 10, fontWeight: 700, color: t.textFaint, textTransform: 'uppercase', letterSpacing: '0.6px', marginBottom: 4 }}>
                  {stat.label}
                </div>
                <div style={{ fontSize: 26, fontWeight: 800, color: stat.color, letterSpacing: '-0.5px' }}>
                  {stat.value}
                </div>
              </div>
            ))}
          ...
          {/* Bucket distribution */}
          <div style={cardStyle}>
            <div style={{ fontSize: 13, fontWeight: 700, color: t.text, marginBottom: 14 }}>Lead Buckets</div>
          ...
          {/* Source distribution */}
          <div style={cardStyle}>
            <div style={{ fontSize: 13, fontWeight: 700, color: t.text, marginBottom: 14 }}>Leads by Source</div>
              ...
              data.leads_by_source.map(src => {
                ...
                return (
                  <div key={src.source} style={{ marginBottom: 10 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                      <span style={{ fontSize: 12, color: t.text }}>{src.source}</span>
                      <span style={{ fontSize: 12, color: t.textMuted }}>{src.count} ({pct}%)</span>
                    </div>
                    <div style={{ height: 6, background: t.border, borderRadius: 3, overflow: 'hidden' }}>
                      <div style={{
